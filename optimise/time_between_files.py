...
def get_creation_time(file_path):
    timestamp = os.path.getmtime(file_path)  # mtime = last edit, not creation
...
prt_dirs = ['op13/rt0.03/threads1','op18/rt0.03/threads2','op18/rt0.03/threads4','op18/rt0.03/threads8','op18/rt0.03/threads16']#,'threads16']
...
for i,prt_dir in enumerate(prt_dirs):
    ...
    label = prt_dir.split("threads")[1] + " threads"
    if prt_dir == 'op13/rt0.03/threads1':
        time_diffs = time_diffs[:13]
        label = "Serial"

    print(f'{prt_dir}: time_diffs {len(time_diffs)}')
    time_diffs_all.append(time_diffs)

    # Plotting
    ax1.plot(range(0,len(time_diffs)),time_diffs, marker='o',label=label)
ax1.set_title("Time Elapsed Between Consecutive Files")
ax1.set_xlabel("File Index",fontsize=big_font)
ax1.legend()
ax1.set_ylabel("Time Elapsed (minutes), cumulative",fontsize=big_font)
ax1.grid(True)
