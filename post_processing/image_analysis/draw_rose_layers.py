...
def draw_rose_plot_double(double_hist: list):
    ...
    ax.bar(np.deg2rad(np.arange(0, 360, 10)), fast_hist,
        width=np.deg2rad(10), bottom=0.0, color=(0.859, 0.271, 0.271),alpha=0.5,
        # edgecolor='k', 
        linewidth=2,zorder=2)    # 10 -> 15

    ax.bar(np.deg2rad(np.arange(0, 360, 10)), slow_hist,
        width=np.deg2rad(10), bottom=0.0, color=(0.722, 0.671, 0.671),alpha=0.5,
        hatch="xx",
        # edgecolor='k', 
        # linewidth=2,
        zorder=2)
...
for tstep in tsteps:
    for fast_or_slow in ["fast","slow"]:
        ...
        print(f'rose_histogram {rose_histogram}')
        ...
        rose_histogram = []
    print(f'rose_histogram_double[0] {rose_histogram_double[0]}')
    print(f'rose_histogram_double[1] {rose_histogram_double[1]}')
