...
def dbscan_and_plot(X,e,min_samples):
    ...
    if len(X)>0:
        ...
        scatter = sns.scatterplot(data=X,x='x coord',y='y coord',hue='cluster',palette="Set2",marker='o',alpha=0.75,s=10,edgecolor='k',linewidth=0)
        sns.color_palette("hls", 8)

        # Calculate the centroids of each cluster or choose representative points
        centroids = X.groupby('cluster').mean().reset_index()
        ...
        scatter.legend(ncol=int(len(unique_labels)/10),loc='center left',bbox_to_anchor=(1.1, 0.5))
