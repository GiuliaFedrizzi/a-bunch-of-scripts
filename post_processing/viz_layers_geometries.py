...
visc = "visc_3_1e3"
mr_contrast_dirs = ['lr43',                 'lr46',                    'lr47',                  'lr44',                        'lr48',                      'lr49']
y_variable = ['lr43: Original Layers, mrc3','lr46: Thick Layers, mrc3','lr47: No central, mrc3','lr44: Original Layers,mrc1.5','lr48: Thick Layers, mrc1.5','lr49: No central, mrc1.5'] 
# mr_contrast_dirs.reverse()
print(f'len y_variable {len(y_variable)}')
def_rate_dirs = ['def0e-8','def5e-7']
melt_labels = '0.005'
whats_constant = "cm"
...
if rose == False:
    ...
    for t in times:
        ...
        ax.set_xticklabels(def_rate_and_time,fontsize=7,rotation=20)

        # y_variable = [str(i) for i in mr_contrast_dirs]   # convert to list of strings
        ax.set_yticklabels(y_variable,fontsize=8)   #  position and values of ticks on the y axis. Start: 441 (half of image height) End: height of image times num of images in one column, Step: 883 (height of image)
        y_ticks_positions = np.arange(im_height/2,im_height*len(y_variable)+im_height/2,im_height)
        ...
        if visc == "visc_2_1e2":
            filename = 'images_in_grid_layers/layer_geom_'+whats_constant+'_v1e2_t'
        elif visc == "visc_3_1e3":
            filename = 'images_in_grid_layers/layer_geom_'+whats_constant+'_v1e3_t'
        else:
...
if rose == True:
    ...
    for t in times:
        ...
        ax.set_ylabel('Melt Rate Contrast')
        if visc == "visc_2_1e2":
            filename = 'images_in_grid_layers/layer_rose_geom_'+whats_constant+'_v1e2_t'
        elif visc == "visc_3_1e3":
            filename = 'images_in_grid_layers/layer_rose_geom_'+whats_constant+'_v1e3_t'
        else:
